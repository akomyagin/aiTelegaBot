...
## 10. Итог (пост-факт, после реализации)

**Статус: реализовано, смержено, задеплоено на прод-VPS, проверено вживую.**
PR [#8](https://github.com/akomyagin/aiTelegaBot/pull/8) → merge в `master`
(`435cab5`). Хотфикс поверх — `4695043` (см. ниже).

### 10.1 Независимое ревью нашло 8 замечаний — все исправлены до мержа

8-угловое ревью (Opus) перед мержем нашло и здесь же исправило:
- **[критично]** пробный `Collect` при `/addsource @канал` (kind=tg_botapi)
  дренировал общий продакшен-буфер каналов (`ChannelBuffer`), теряя уже
  накопленные посты других managed-источников → для `tg_botapi` валидация
  теперь не трогает буфер, полагается только на проверку админства
  (`isBotAdmin`, уже была на этапе `detectKind`).
- **[критично]** `channelBuf` создаётся безусловно (нужно для `/addsource`
  без рестарта) и ничем не ограничивался → добавлен cap на 500 элементов
  (`internal/telegram/buffer.go`, `maxBufferedItems`).
- скрапинг публичных каналов (`PublicSource.Collect`) молча глотал сетевые
  ошибки → валидный канал мог быть отклонён при временном сбое сети →
  добавлен `PublicSource.TrialCollect`, не глушащий ошибку, используется
  только при валидации в `/addsource`.
- `/addsource` не проверял дубли по `(kind, ref)` → добавлена проверка.
- **Известное ограничение, НЕ устранено** (задокументировано комментарием в
  `internal/app/dynamic_sources.go`): общий `channelBuf` на несколько
  managed-каналов (`tg_botapi`) отдаёт все посты первому вызвавшему
  `Collect` за прогон, остальные получают пусто. Предсуществовал и раньше
  при ≥2 статических managed-каналах — эта фича лишь расширила поверхность
  (теперь можно добавить второй managed-источник командой). Триггер для
  возврата: если понадобится больше одного активного `tg_botapi`-источника
  одновременно — нужна per-channel буферизация (ключ — username канала).

### 10.2 Деплой на прод-VPS вскрыл ещё один баг — живым тестом

При первом реальном деплое (см. README «Если VPS совсем маленький») нашёлся
баг, который не поймали ни юниты, ни ревью: ответы бота уходят с
`ParseMode: HTML`, а тексты-подсказки содержали буквальные `<url|@channel>` —
Telegram пытался распарсить их как HTML-теги и **отклонял всё сообщение
целиком**. Из-за этого не отвечали даже `/start`/`/help`, не только новые
команды. Исправлено заменой `<...>` на `[...]` во всех текстах команд
(`internal/telegram/router.go`, `internal/telegram/source_admin.go`) —
коммит `4695043`, запушен напрямую в `master` по решению пользователя (баг
найден и исправлен уже после мержа PR).

**Вывод:** юнит-тесты и ревью кода не поймают проблемы, специфичные для
Telegram Bot API (ParseMode, лимиты, экранирование) — для новых
пользовательских текстов ответа бота стоит держать в уме, что `<`/`>`/`&`
имеют смысл в HTML-режиме.

### 10.3 Живая проверка на реальном боте (после хотфикса)

- `/addsource @bbbreaking` (публичный канал, бот не админ) → `tg_public`,
  сохранён.
- `/addsource https://hnrss.org/frontpage` → `rss`, сохранён.
- Дайджест сразу после добавления первого источника вырос с 14 до 20
  постов — источник подхватился без рестарта бота, как и задумано.
- `/removesource 1` → `bbbreaking` помечен `enabled=0` в БД, второй
  источник не тронут.

Всё подтверждено прямым чтением таблицы `sources` в SQLite на сервере.
